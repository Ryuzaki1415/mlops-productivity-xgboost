...
# Be direct and specific. Do not give generic wellness advice. Reference the actual numbers."""
    ...
    prompt2 = f"""Productivity score: {score:.1f}/100 ({category}).
    Sleep {raw_features['Sleep_Hours']}h (deficit {max(0, 8 - raw_features['Sleep_Hours']):.1f}h), stress {raw_features['Stress_Level']}/10, phone {raw_features['Daily_Phone_Hours']}h/day, social media {raw_features['Social_Media_Hours']}h, caffeine {raw_features['Caffeine_Intake_Cups']} cups, weekend screen {raw_features['Weekend_Screen_Time_Hours']}h, occupation {raw_features['Occupation']}.
    SHAP drivers: {contributors_text}

    Reply in EXACTLY 3 sentences. No bullet points. No headers. No lists.
    Sentence 1: What is driving this score (use specific numbers).
    Sentence 2: The single biggest factor to change.
    Sentence 3: Two concrete actions tailored to their profile."""
    return prompt2
